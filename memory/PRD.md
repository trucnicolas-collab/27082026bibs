...
- [x] **20 nuits de pose max** (au lieu de 16) : `MAX_ES_NIGHTS=20`, valeurs standard `ALLOWED_ES_NIGHTS=[10,12,14,16,18,20]`, input `max=20` + clamp 20, avertissements fourchette 10-20. Suggestion auto : pour >16 nuits, répartition sur **4 semaines de 4-5 nuits** (pour rester sur les 4 slides "semaine" du PPTX). 5ᵉ couleur (violet) ajoutée aux 3 palettes (frontend WEEK_COLORS, backend WEEK_NIGHT_PALETTE + pptx WEEK_COLORS_HEX). PPTX : tableau date des slides semaine élargi dynamiquement (`_ensure_table_cols`) pour afficher jusqu'à 5 nuits/semaine. Validé e2e : 20 nuits → 4 slides semaine (S1:1→5 … S4:16→20), exports PPTX/RTR/Carrefour = 200, pytest 8/8.
