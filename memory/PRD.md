...
- [x] **(05/07/2026) Sécurisation ligne « Support individuel alu SA »** : quand le fichier importé ne contient PAS cette ligne, la sélection surface la crée désormais avec une **référence valide** (`SUPPORT_ALU_SA_REF = "16000"`, modifiable) au lieu d'une référence vide → l'export n'est plus bloqué. La ligne est 100% saisonnière (6000 pour +10000 m², 4000 pour −10000 m²). Quand la ligne existe déjà dans le fichier, comportement inchangé : le delta incrémente sa colonne `saisonnier` (réf d'origine conservée). Fichier : `backend/server.py` (`SUPPORT_ALU_SA_REF`, `_create_surface_added` param `reference`). Validé : ligne créée réf=16000/saison=6000, exports Carrefour + RTR = 200, pytest test_export_carrefour 8/8.
