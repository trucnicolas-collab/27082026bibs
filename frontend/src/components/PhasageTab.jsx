...
export default function PhasageTab({ uploadId }) {
    ...
    const nightTotals = useMemo(() => {
        const tot = {};
        for (let n = 1; n <= nbNuits; n++) tot[n] = { es_15: 0, es_21: 0, rails_es: 0 };
        rows.forEach((r) => {
            if (!r.nuit) return;
            ...
            <div className="flex-1 overflow-auto custom-scroll p-3">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-4">
                    ...
                    <div data-testid="phasage-left-table">
                                        ...
                                        return (
                                            <tr key={r.id} className="border-t border-gray-100 hover:bg-gray-50">
                                                <td className="px-1 py-1">
                                                    <input
                                                        list={`allees-list-${uploadId}`}
                                                        value={r.allee}
                                                        onChange={(e) => updateRow(r.id, { allee: e.target.value })}
                                                        data-testid={`row-allee-${r.id}`}
                                                        placeholder="Sélectionner…"
                                                        className="w-full h-6 px-1.5 text-xs border border-gray-200 rounded focus:ring-1 focus:ring-[#056839] focus:border-[#056839] outline-none font-mono-data"
                                                    />
                                                </td>
                        ...
                        <datalist id={`allees-list-${uploadId}`}>
                            {alleeOptions.map((a) => <option key={a} value={a} />)}
                        </datalist>
                    </div>
                    ...
                    <div data-testid="phasage-right-table">
                        ...
                        <div className="border border-gray-200 rounded overflow-hidden">
                            <table className="w-full text-xs">
                                ...
                                <tbody>
                                    {Array.from({ length: nbNuits }, (_, i) => i + 1).map((n) => {
                                        const t = nightTotals[n] || { es_15: 0, es_21: 0, rails_es: 0 };
