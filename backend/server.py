...
MOQ_BY_REF: dict[str, int] = {
    "11892": 5,   "12202": 10,  "13469": 100, "13585": 48,  "13827": 25,
    "14218": 5,
    ...
    "17165": 25,  "17285": 24,  "17651": 25,  "17673": 100, "17717": 25,
    "17723": 1,   "17724": 100, "17740": 100, "17741": 50,  "17868": 24,
    "17869": 100,
    "17870": 100,
    ...
    "3962": 50,   "3966": 50,   "3971": 50,   "4507": 100,  "4552": 500,
    "6669": 1,    "9484": 50,
}
...
APP_BUILD_TAG = "pose-geoloc-clear-labels-2026-02-12-v8"
...
def _is_sa_42(desig: str) -> bool:
    """Détecte les étiquettes SA 4.2 / 4.2 WP (famille séparée, traitée
    comme le freezer). Critère : désignation SA contenant '4.2'."""
    d = _norm_desig(desig)
    return "4.2" in d or "4,2" in d
...
def compute_phasage_summary(d: dict) -> dict:
    ...
    for r in raw_records:
        ...
        node = by_allee.setdefault(composite_key, {
            ...
            "sa_42": 0.0,      # SA 4.2 / 4.2 WP (famille séparée)
            "rails_es": 0.0,
            "cameras": 0.0,
            "camera_elems": [],
    ...
    for a in allees:
        ...
        s = sa_by_secteur.setdefault(sec, {"secteur": sec, "sa_15": 0, "sa_21_std": 0, "sa_21_freezer": 0, "sa_42": 0, "rayons": {}})
        rnode = s["rayons"].setdefault(ray, {"rayon": ray, "sa_15": 0, "sa_21_std": 0, "sa_21_freezer": 0, "sa_42": 0, "allees": []})
        for k in ("sa_15", "sa_21_std", "sa_21_freezer", "sa_42"):
            v = a.get(k, 0) or 0
            rnode[k] += v
            s[k] += v
        ...
        a_42 = a.get("sa_42", 0) or 0
        if (a_15 + a_21 + a_fz + a_42) > 0:
            rnode["allees"].append({
                "allee": a.get("allee") or "",
                "sa_15": a_15, "sa_21_std": a_21, "sa_21_freezer": a_fz, "sa_42": a_42,
            })
    sa_breakdown = []
    for sec in sorted(sa_by_secteur.keys()):
        s = sa_by_secteur[sec]
        rayons = [s["rayons"][r] for r in sorted(s["rayons"].keys())]
        rayons = [r for r in rayons if (r["sa_15"] + r["sa_21_std"] + r["sa_21_freezer"] + r["sa_42"]) > 0]
        if not rayons:
            continue
        sa_breakdown.append({
            "secteur": sec,
            "sa_15": s["sa_15"], "sa_21_std": s["sa_21_std"], "sa_21_freezer": s["sa_21_freezer"], "sa_42": s["sa_42"],
            "rayons": rayons,
        })
...
def _write_phasage_sheet(workbook, writer, d, fmt_header, fmt_cell, fmt_total):
    ...
    ws.set_column(10, 10, 12) # K EEG ES
    ws.set_column(11, 11, 12) # L Rails ES
    ws.set_column(12, 12, 10) # M SA 1.5
    ws.set_column(13, 13, 10) # N SA 2.1
    ws.set_column(14, 14, 11) # O SA 2.1 frz
    ...
    headers_left = ["N° Allée", "EEG ES", "Rails ES", sa_col_label, "Nuit"]
    ...
    col_right = 6
    NB_RIGHT_COLS = 11  # Nuit|Date|SR|Allées|EEG ES|Rails ES|SA 1.5|SA 2.1|SA 2.1 frz|4.2/4.2 WP|Caméras
    ws.merge_range(start_left, col_right, start_left, col_right + NB_RIGHT_COLS - 1, "Récap par nuit", fmt_title)
    headers_right = ["Nuit", "Date", "Secteur/Rayon", "Allées", "EEG ES", "Rails ES", "SA 1.5", "SA 2.1", "SA 2.1 frz", "4.2/4.2 WP", "Caméras"]
    ...
    # SA à installer par nuit, détaillé par type (statique — cohérent avec les
    # autres onglets Récap/Semaine/Phasage full). Utilise compute_node_sa_install.
    sa_by_night: dict[int, dict] = {n: {"sa_15": 0.0, "sa_21": 0.0, "freezer": 0.0, "sa_42": 0.0}
                                    for n in range(1, nb_nuits + 1)}
    for _r2 in rows_assign:
        _n2 = _r2.get("nuit")
        _auid = str(_r2.get("allee") or "").strip()
        if not _n2 or not _auid:
            continue
        try:
            _ni = int(_n2)
        except (ValueError, TypeError):
            continue
        if not (1 <= _ni <= nb_nuits):
            continue
        _node = _resolve_idx_node(_auid, idx_allees_full)
        if not _node:
            continue
        _inst = compute_node_sa_install(_node, cfg_sa)
        _b = sa_by_night[_ni]
        _b["sa_15"] += _inst["sa_15"]; _b["sa_21"] += _inst["sa_21"]
        _b["freezer"] += _inst["freezer"]; _b["sa_42"] += _inst["sa_42"]
    ...
    for i, n in enumerate(range(1, nb_nuits + 1), start=0):
        ...
        _sb = sa_by_night.get(n, {})
        ws.write_number(rrow, col_right + 6, int(round(_sb.get("sa_15") or 0)), fmt_num_neutral)
        ws.write_number(rrow, col_right + 7, int(round(_sb.get("sa_21") or 0)), fmt_num_neutral)
        ws.write_number(rrow, col_right + 8, int(round(_sb.get("freezer") or 0)), fmt_num_neutral)
        ws.write_number(rrow, col_right + 9, int(round(_sb.get("sa_42") or 0)), fmt_num_neutral)
        # Colonne Caméras (col_right + 10)
        ...
        if cam_val > 0:
            ws.write_number(rrow, col_right + 10, cam_val, fmt_cam_neutral)
        else:
            ws.write_blank(rrow, col_right + 10, None, fmt_cam_neutral)
    ...
    ws.write_formula(rrow_total, col_right + 3,
                     ...
                     fmt_total_lbl)
    for offset in range(4, NB_RIGHT_COLS):  # EEG ES, Rails, SA 1.5/2.1/frz/4.2, Caméras
            ...
            for n in range(n_start, n_end + 1):
                if n > nb_nuits:
                    ...
                    ws.write_formula(cur_row, col_right + 5,
                                     f'=SUMIFS({C_range},{E_range},"{nuit_label}")', fmt_num_neutral)
                    _sbw = sa_by_night.get(n, {})
                    ws.write_number(cur_row, col_right + 6, int(round(_sbw.get("sa_15") or 0)), fmt_num_neutral)
                    ws.write_number(cur_row, col_right + 7, int(round(_sbw.get("sa_21") or 0)), fmt_num_neutral)
                    ws.write_number(cur_row, col_right + 8, int(round(_sbw.get("freezer") or 0)), fmt_num_neutral)
                    ws.write_number(cur_row, col_right + 9, int(round(_sbw.get("sa_42") or 0)), fmt_num_neutral)
                    # Caméras
                    fmt_cam_week = workbook.add_format({"border": 1, "align": "right",
                                                         "bold": True, "font_color": "#6B21A8"})
                    cam_val_w = int(round(cam_per_night.get(n, 0)))
                    if cam_val_w > 0:
                        ws.write_number(cur_row, col_right + 10, cam_val_w, fmt_cam_week)
                    else:
                        ws.write_blank(cur_row, col_right + 10, None, fmt_cam_week)
                cur_row += 1
...
def _write_phasage_full_sheet(workbook, writer, d, fmt_header, fmt_cell, fmt_total):
    ...
    for r in es_plan.get("rows") or []:
        ...
        dn["sa_inst_42"] += inst["sa_42"]
        dn["es"] += inst["sa_15"] + inst["sa_21"] + inst["freezer"] + inst["sa_42"]
        dn["sa_mag"] += max(0.0, node_sa_total(node) - (inst["sa_15"] + inst["sa_21"] + inst["freezer"] + inst["sa_42"]))
    for r in cam_plan.get("rows") or []:
        n, a_uid = r.get("nuit"), str(r.get("allee") or "").strip()
        if not n or not a_uid: continue
        node = _resolve_idx_node(a_uid, idx)
    ...
    # Row 0 : titre global (fusionné A:N)
    ws.merge_range(0, 0, 0, 13, "Phasage full — Planning consolidé", fmt_title)
    # Row 1 : sous-titres distincts pour chaque bloc
    ws.merge_range(1, 0, 1, 8, "Phasage étiquettes et rails", fmt_subtitle_es)
    ws.merge_range(1, 9, 1, 10, "Nuit", fmt_subtitle_nuit)
    ws.merge_range(1, 11, 1, 13, "Phasage caméras", fmt_subtitle_cam)
    # Row 2 : en-têtes de colonnes
    headers = ["Allées", "ES", "Rails ES", "SA 1.5", "SA 2.1", "SA 2.1 frz", "4.2/4.2 WP", "SA magasin", "Secteur/Rayon",
               "Nuit", "Date",
    ...
    for n in sorted_nuits:
        ...
        if info["es_allees"]:
            ...
            ws.write_number(r, 6, round(info["sa_inst_42"]), fmt_num)
            ws.write_number(r, 7, round(info["sa_mag"]), fmt_num)
        else:
            for c in range(0, 8):
                ws.write_blank(r, c, None, fmt_num)
        # Secteur/Rayon EEG (col I)
        sr_list_es = sr_by_nuit_es.get(n) or []
        ws.write_string(r, 8, _format_sr_grouped(sr_list_es), fmt_sr_cell)
        # Nuit (col J, BLANCHE par fmt_nuit)
        ws.write_number(r, 9, n, fmt_nuit)
        # Date (col K)
        date_iso = dates_map.get(str(n))
        if date_iso:
            try:
                from datetime import datetime as _dt
                ws.write_datetime(r, 10, _dt.strptime(date_iso, "%Y-%m-%d").date(), fmt_date_cell)
            except Exception:
                ws.write_string(r, 10, date_iso, fmt_date_cell)
        else:
            ws.write_blank(r, 10, None, fmt_date_cell)
        # Secteur/Rayon Cam (col L)
        sr_list_cam = sr_by_nuit_cam.get(n) or []
        ws.write_string(r, 11, _format_sr_grouped(sr_list_cam), fmt_sr_cell)
        # Bloc Cam (cols M-N)
        if info["cam_allees"]:
            ws.write_string(r, 12, ", ".join(info["cam_allees"]), fmt_text)
            ws.write_number(r, 13, round(info["cam"]), fmt_num)
        else:
    ...
    if r > 3:
        ...
        ws.write_formula(r, 7, f"=SUM(H{first_excel}:H{last_excel})", fmt_total_row)
        ws.write(r, 8, "", fmt_total_lbl)  # Secteur EEG
        ws.write_formula(r, 9, f'=COUNTA(J{first_excel}:J{last_excel})&" nuits"', fmt_total_lbl)
        ws.write(r, 10, "", fmt_total_lbl)
        ws.write(r, 11, "", fmt_total_lbl)  # Secteur Cam
        ws.write(r, 12, "", fmt_total_lbl)
        ws.write_formula(r, 13, f"=SUM(N{first_excel}:N{last_excel})", fmt_total_row)

        # Mise en forme conditionnelle par nuit : colore TOUTES les colonnes
        # SAUF la Nuit (col J), conformément à la demande du 06/02/2026.
        for night_num in range(1, max(sorted_nuits) + 1):
            ...
            # A:I (cols 0..8) — bloc ES + Secteur EEG
            ws.conditional_format(3, 0, last_excel - 1, 8,
                {"type": "formula",
                 "criteria": f'=$J4={night_num}',
                 "format": fmt_night})
            # K (col 10) — Date seule (skip Nuit en col J)
            ws.conditional_format(3, 10, last_excel - 1, 10,
                {"type": "formula",
                 "criteria": f'=$J4={night_num}',
                 "format": fmt_night})
            # L:N (cols 11..13) — Secteur Cam + Allées Cam + Caméras
            ws.conditional_format(3, 11, last_excel - 1, 13,
                {"type": "formula",
                 "criteria": f'=$J4={night_num}',
                 "format": fmt_night})
...
def _write_recap_par_nuit_sheet(wb, writer, agg):
    ...
    cols = ["Nuit", "Date", "Secteur/Rayon", "Allées", "EEG ES", "Rails ES", "SA 1.5", "SA 2.1", "SA 2.1 frz", "4.2/4.2 WP", "Caméras"]
    widths = [10, 12, 28, 36, 12, 12, 10, 10, 11, 11, 12]
    ws.merge_range(0, 0, 0, len(cols) - 1, "Récap par nuit", fmt_h)
    for ci, (h, w) in enumerate(zip(cols, widths)):
        ws.write(1, ci, h, fmt_lbl)
        ws.set_column(ci, ci, w)

    tot_15 = tot_21 = tot_fz = tot_42 = tot_eeg = tot_rails = tot_cam = 0
    row = 2
    for n in range(1, n_es + 1):
        ...
        safz = int(round(b.get("sa_inst_freezer") or 0))
        sa42 = int(round(b.get("sa_inst_42") or 0))
        ...
        ws.write(row, 8, safz, p["right"])
        ws.write(row, 9, sa42, p["right"])
        if cam:
            ws.write(row, 10, cam, p["right"])
        else:
            ws.write_blank(row, 10, None, p["right"])
        tot_eeg += eeg; tot_rails += rails; tot_15 += sa15; tot_21 += sa21; tot_fz += safz; tot_42 += sa42; tot_cam += cam
        row += 1
    tot_row = row
    ...
    ws.write(tot_row, 8, tot_fz, fmt_total_n)
    ws.write(tot_row, 9, tot_42, fmt_total_n)
    ws.write(tot_row, 10, tot_cam, fmt_total_n)
    ws.freeze_panes(2, 0)


def _write_week_sheets(wb, writer, agg):
    """Une feuille par semaine (« Semaine S1 », …) — IDENTIQUE au tableau
    semaine du PPTX : Nuit · Date · Secteur/Rayon · Allées · EEG · Rails ES ·
    colonnes SA à installer dynamiques (1.5 / 2.1 / 2.1 frz selon ce qui est
    posé) + « SA » (magasin, hors phasage) en italique si présente + ligne
    « Sous-total S{n} »."""
    weeks_es = agg.get("weeks_es") or []
    if not weeks_es:
        return
    es = agg["es_per_nuit"]
    ...
    fmt_sub_n = wb.add_format({"bold": True, "bg_color": "#E5E7EB", "border": 1, "align": "right"})
    fmt_sub_i = wb.add_format({"bold": True, "italic": True, "font_color": "#6B7280",
                               "bg_color": "#E5E7EB", "border": 1, "align": "right"})

    def _pal(n, italic=False):
        color = night_color_hex(int(n), weeks_es)
        return wb.add_format({"bg_color": color, "border": 1, "align": "right",
                              "italic": italic, "font_color": ("#6B7280" if italic else "black")})

    cur = 1
    for wi, w in enumerate(weeks_es):
        ww = int(w or 0)
        if ww <= 0:
            continue
        ...
        if sum((b.get("sa_inst_42") or 0) for b in wk) > 0:
        ...
        ws.merge_range(0, 0, 0, len(cols) - 1, f"Plan de phasage EEG et rails par nuit – S{wi + 1}", fmt_h)
        for ci, (h, wd) in enumerate(zip(cols, widths)):
            ws.write(1, ci, h, fmt_lbl)
            ws.set_column(ci, ci, wd)
        row = 2
        sub = {"eeg": 0, "rails": 0}
        for c in sa_cols:
            sub[c[0]] = 0
        for n in nights:
            b = es.get(n, {})
            col_c = night_color_hex(int(n), weeks_es)
            cfmt = wb.add_format({"bg_color": col_c, "border": 1, "align": "center", "bold": True})
            lfmt = wb.add_format({"bg_color": col_c, "border": 1, "align": "left", "font_size": 9, "text_wrap": True})
            dfmt = wb.add_format({"bg_color": col_c, "border": 1, "align": "center", "num_format": "dd/mm/yyyy"})
            ws.write(row, 0, f"Nuit {n}", cfmt)
            d_iso = agg["dates"].get(str(n))
            if d_iso:
                try:
                    ws.write_datetime(row, 1, datetime.strptime(d_iso, "%Y-%m-%d").date(), dfmt)
                except Exception:
                    ws.write_string(row, 1, d_iso, dfmt)
            else:
            ...
            for j, (h, keys, ital) in enumerate(sa_cols):
                v = int(round(sum((b.get(k) or 0) for k in keys)))
                sub[h] += v
                if v:
                    ws.write(row, 6 + j, v, _pal(n, ital))
                else:
                    ws.write_blank(row, 6 + j, None, _pal(n, ital))
            row += 1
        # Sous-total
        ws.write(row, 0, f"Sous-total S{wi + 1}", fmt_sub_lbl)
        for c in (1, 2, 3):
            ws.write(row, c, "", fmt_sub_lbl)
        ws.write(row, 4, sub["eeg"], fmt_sub_n)
        ws.write(row, 5, sub["rails"], fmt_sub_n)
        for j, (h, keys, ital) in enumerate(sa_cols):
            ws.write(row, 6 + j, sub[h], fmt_sub_i if ital else fmt_sub_n)
        ws.freeze_panes(2, 0)
...
async def _build_export(d: dict, sheet: str = "all"):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        if sheet in ("all", "recap_nuit"):
            ...
            _write_week_sheets(workbook, writer, _agg_rn)
...
class SaInstallConfig(BaseModel):
    ...
    sa_42: bool = False            # inclure SA 4.2 / 4.2 WP (toutes)
...
def node_sa_total(node: dict) -> float:
    ...
    if n21 is None:
        n21 = max(0.0, float(node.get("sa_21") or 0) - float(node.get("sa_21_freezer") or 0))
    return float(node.get("sa_15") or 0) + float(n21) + float(node.get("sa_21_freezer") or 0) + float(node.get("sa_42") or 0)
...
def _aggregate_phasage_for_export(d: dict) -> dict:
    ...
    return {
        ...
        "totals_es": {
            ...
            "sa_inst_42": sum(b["sa_inst_42"] for b in es_per_nuit.values()),
...
async def _build_carrefour_export(d: dict):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        _write_week_sheets(wb, writer, agg)
        ...
        ws.merge_range(0, 0, 0, 8, "Phasage étiquettes et rails", fmt_h_eeg)
        ws.merge_range(0, 9, 0, 10, "Nuit", fmt_h)
        ws.merge_range(0, 11, 0, 13, "Phasage caméras", fmt_h_cam)
        headers5 = ["Allées", "ES", "Rails ES", "SA 1.5", "SA 2.1", "SA 2.1 frz", "4.2/4.2 WP",
                    "SA magasin", "Secteur/Rayon EEG",
                    "Nuit", "Date",
                    "Secteur/Rayon Cam", "Allées", "Caméras"]
        widths5 = [22, 10, 10, 9, 9, 10, 10, 11, 20, 8, 11, 20, 22, 12]
        ...
        for i, n in enumerate(range(1, n_es + 1), start=0):
            ...
            ws.write(row, 6, int(round(es.get("sa_inst_42") or 0)), p_es.get("right", fmt_num))
            ws.write(row, 7, int(round(es.get("sa_mag") or 0)), p_es.get("right", fmt_num))
            ws.write(row, 8, _format_sr_grouped(es.get("secteurs_rayons") or []),
                     p_es.get("sr", fmt_cell))
            # Nuit (BLANCHE — seule colonne sans fond)
            ws.write(row, 9, f"{n}", wb.add_format({
            ...
            if d_iso:
                try:
                    ws.write_datetime(row, 10, datetime.strptime(d_iso, "%Y-%m-%d").date(),
                                      p_es.get("date", fmt_date))
                except Exception:
                    ws.write_string(row, 10, d_iso, p_es.get("date", fmt_date))
            else:
                ws.write_blank(row, 10, None, p_es.get("date", fmt_date))
            # Bloc Cam
            ws.write(row, 11, _format_sr_grouped(cam.get("secteurs_rayons") or []),
                     p_cam.get("sr", fmt_cell) if cam else fmt_cell)
            ws.write(row, 12, ", ".join(str(a) for a in (cam.get("allees") or [])),
                     p_cam.get("left", fmt_cell) if cam else fmt_cell)
            if cam.get("cam"):
                ws.write(row, 13, int(round(cam.get("cam") or 0)),
                         p_cam.get("right", fmt_num))
            else:
                ws.write_blank(row, 13, None, fmt_cell)
        ...
        ws.write(tot_row, 6, int(round(agg["totals_es"]["sa_inst_42"])), fmt_total_n)
        ws.write(tot_row, 7, int(round(agg["totals_es"]["sa_mag"])), fmt_total_n)
        ws.write(tot_row, 8, "", fmt_total_lbl)
        ws.write(tot_row, 9, f"{n_es} nuits", wb.add_format({
            "border": 1, "align": "center", "bold": True, "bg_color": "#FFFFFF"
        }))
        ws.write(tot_row, 10, "", fmt_total_lbl)
        ws.write(tot_row, 11, "", fmt_total_lbl)
        ws.write(tot_row, 12, f"{n_cam} nuits cam", fmt_total_lbl)
        ws.write(tot_row, 13, int(round(agg["totals_cam"]["cam"])), fmt_total_n)
        ws.freeze_panes(2, 0)
